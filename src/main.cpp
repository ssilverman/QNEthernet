...
static constexpr Reg kRegList[kRegCount]{
    { 0x0456, 0x0003, 9 },
    { 0x0411, 0x001f, 0 },
    { 0x0416, 0x000f, 4 },
    { 0x0190, 0x1fff, 0 },
...
static constexpr uint16_t kSegmentRegs[kSegmentCount][kRegCount]{
    { 0x0003, 0x0013, 0x000a, 0x1c12, 0x0d07, 0x1004, 0x0002 },
    { 0x0003, 0x0013, 0x000a, 0x1c22, 0x0d14, 0x1004, 0x0002 },
    { 0x0003, 0x0015, 0x000a, 0x1e32, 0x0d14, 0x1004, 0x0002 },
    { 0x0003, 0x0016, 0x000a, 0x1e42, 0x343b, 0x1008, 0x0002 },
...
static void runTests() {
  ...
  for (uint16_t seg = 1; seg <= 5; seg++) {
    ...
    for (size_t i = 0; i < kRegCount; i++) {
      ...
      const uint16_t r =
          clearAndSet16(driver::mdio_read(reg.addr),
                        (reg.mask << reg.shift),
                        (kSegmentRegs[seg - 1][i] & reg.mask) << reg.shift);
      driver::mdio_write(reg.addr, r);
    }
