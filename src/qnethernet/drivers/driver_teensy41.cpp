  ...
  if ((pBD->status & err_mask) != 0) {
    ...
    if (p != nullptr) {
      ...
      if (p->timestampValid) {
        ...
        p->timestamp.tv_nsec = static_cast<long>(pBD->timestamp);
      }
    } else {
      LINK_STATS_INC(link.drop);
...
static inline void update_bufdesc(volatile BufferDescriptor* const pBD,
  ...
  if (s_doTimestampNext) {
    ...
    pBD->extend1 &= static_cast<uint16_t>(~tx_bd_extend1::kTimestamp);
  }
...
static void enet_isr() {
  ...
  if (ENET::EIR::TS_AVAIL != 0) {
    ...
    s_txTimestamp.tv_nsec = static_cast<long>(*ENET::ATSTMP::TIMESTAMP);
...
bool ieee1588_read_timer(struct timespec* const t) {
  ...
  t->tv_nsec = static_cast<long>(*ENET::ATVR::ATIME);
...
bool ieee1588_write_timer(const struct timespec* const t) {
  ...
  ENET::ATVR::ATIME = static_cast<uint32_t>(t->tv_nsec);
  qnethernet_hal_enable_interrupts();  // }
...
bool ieee1588_set_channel_mode(const size_t channel, const int mode) {
  ...
  *tcsr = r & ~(ENET::CHANNEL::TCSR::vals::TMODE.kMask |
  ...
  r = (r & ~(ENET::CHANNEL::TCSR::vals::TMODE.kMask |
             ENET::CHANNEL::TCSR::vals::TF(1))) |  // Don't clear TF (w1c)
...
bool ieee1588_set_channel_output_pulse_width(const size_t channel,
  ...
  *tcsr = r & ~(ENET::CHANNEL::TCSR::vals::TMODE.kMask |
  ...
  r = (r & ~(ENET::CHANNEL::TCSR::vals::TPWC.kMask |
             ENET::CHANNEL::TCSR::vals::TF(1))) |  // Don't clear TF (w1c)
