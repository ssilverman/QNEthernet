...
bool EthernetIEEE1588Class::readTimer(std::timespec& t) const {
  return driver::ieee1588_read_timer(&t);
}

bool EthernetIEEE1588Class::writeTimer(const std::timespec& t) const {
...
bool EthernetIEEE1588Class::readAndClearTxTimestamp(
    std::timespec& timestamp) const {
  return driver::ieee1588_read_and_clear_tx_timestamp(&timestamp);
}
...
bool EthernetIEEE1588Class::setChannelMode(size_t channel,
  ...
  return driver::ieee1588_set_channel_mode(channel, static_cast<int>(mode));
}
